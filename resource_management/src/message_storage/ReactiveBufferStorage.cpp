...
namespace resource_management {
  ...
  ReactiveBufferStorage::ReactiveBufferStorage(const std::vector<std::string>& names) : buffers_names_(names)
  {
    for(const auto& name : buffers_names_)
      buffers_[name] = std::make_shared<ReactiveBuffer>(name);
  }

  void ReactiveBufferStorage::setPriority(const std::string& name, FocusPriority_e priority)
  {
      ...
      if((*buffers_[it])())
      {
        if((int)buffers_[it]->getPriority() >= 0) // test if buffer is inhibit
        {
          if((int)(*buffers_[it])()->getPriority() >= 0) // test if message if void
          {
            ...
            max_priority = std::max<double>(tmp, max_priority);
          }
        }
      }
    }

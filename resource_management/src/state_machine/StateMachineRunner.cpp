...
namespace resource_management {
  ...
  StateMachineRunner::StateMachineRunner(float rate) : us_sleep_time_((uint32_t)(1000000.0 / rate)),
                                                       time_out_(compat::rm_ros::Duration(-1)),
                                                       begin_dead_line_(compat::rm_ros::Time(0)),
  ...
  void StateMachineRunner::run()
  {
    ...
    compat::rm_ros::Time start_time = compat::rm_ros::Node::get().currentTime();
    if(begin_dead_line_ != compat::rm_ros::Time(0))
    ...
    while((internal_state_.state_ != nullptr) && (compat::rm_ros::Node::ok()))
    {
      internal_state_mutex_.lock();

      // tets duration end condition
      compat::rm_ros::Time now = compat::rm_ros::Node::get().currentTime();
      if((time_out_ != compat::rm_ros::Duration(-1)) && (now - start_time >= time_out_))
      {
        internal_state_.state_ = nullptr;
  ...
  void StateMachineRunner::runOnceNoEvent()
  {
    ...
    TranstitionState_e tmp_transition = internal_state_.state_->update(&tmp_state);
  ...
  void StateMachineRunner::runOnceWithEvents(std::queue<std::string>& events)
  {
    while(events.empty() == false)
    {
      ...
      StateMachineState* tmp_state = internal_state_.state_;
      TranstitionState_e tmp_transition = internal_state_.state_->update(&tmp_state, event);

  ...
  bool StateMachinesStorage::remove(uint32_t id)
  {
    ...
    for(size_t i = 0; i < states_storage_.size();)
      if(states_storage_[i]->getId() > id)
      {
        ...
        std::cout << "[" << compat::rm_ros::Node::get().getName() << "] remove " << id << "; " << states_storage_.size() << " state machines waiting" << std::endl;
        found = true;
      }
      else
        i++;
  ...
  void StateMachinesStorage::clean()
  {
    for(size_t i = 0; i < states_storage_.size();)
    {
      if(states_storage_[i]->isTooLate())
      {
        ...
        states_storage_.erase(states_storage_.begin() + i);
        std::cout << "[" << compat::rm_ros::Node::get().getName() << "] remove " << internal_state.state_machine_id << "; " << states_storage_.size() << " state machines waiting" << std::endl;
      }
      else
        i++;
    }

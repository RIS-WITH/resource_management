...
namespace resource_synchronizer {
  ...
  void StateMachinesManager::insert(int id, compat::MetaStateMachineHeader header)
  {
    headers_[id] = header;

    if(compat::rs_ros::Duration(header.timeout) == compat::rs_ros::Duration(0))
  ...
  void StateMachinesManager::run()
  {
    ...
    while(run_ && compat::rs_ros::Node::get().ok())
    {
      mutex_.lock();
      applyConstraints();
      init();
      ...
      for(size_t i = 0; i < ids_.size(); i++)
      {
        if(ids_[i].size())
        {
          ...
          else if(running_id == -1)
          {
            ...
            sm_start_time_[ids_[i][0]] = compat::rs_ros::Node::get().currentTime();
  ...
  void StateMachinesManager::applyConstraints()
  {
    for(const auto& it : headers_)
    {
      ...
      {
        if(compat::rs_ros::Duration(it.second.timeout) != compat::rs_ros::Duration(-1))
        {
          auto st_it = sm_start_time_.find(it.first);
          if(st_it != sm_start_time_.end())
          {
            if(compat::rs_ros::Node::get().currentTime() - compat::rs_ros::Time(st_it->second) >= compat::rs_ros::Duration(it.second.timeout))
            {
      ...
      else if((compat::rs_ros::Time(it.second.begin_dead_line) != compat::rs_ros::Time(0)) &&
              (compat::rs_ros::Time(it.second.begin_dead_line) <= compat::rs_ros::Node::get().currentTime()))
      {

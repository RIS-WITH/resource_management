...
namespace resource_management {
  ...
  template<typename StateMachineType, typename StateMachineExtractType, typename... InputDataTypes>
  ResourceManager<StateMachineType, StateMachineExtractType, InputDataTypes...>::ResourceManager(ros::NodeHandlePtr nh, std::vector<std::string> reactiveInputNames, const std::vector<std::string>& pluginsNames, bool synchronized) : _nh(std::move(nh)), _loader("resource_management", "resource_management::EventsInterface")
  {
  ...
  void ResourceManager<StateMachineType, StateMachineExtractType, InputDataTypes...>::prioritiesCallback(const resource_management_msgs::PrioritiesSetter& msg)
  {
    ...
    FocusPriority_e priority = background;
    for(size_t i = 0; i < min; i++)
    {
      switch(msg.values[i])
      {
  ...
  template<typename StateMachineType, typename StateMachineExtractType, typename... InputDataTypes>
  bool ResourceManager<StateMachineType, StateMachineExtractType, InputDataTypes...>::stateMachineCancel(resource_management_msgs::StateMachinesCancel::Request& req,
                                                                                                         resource_management_msgs::StateMachinesCancel::Response& res)
  {
    bool found = false;

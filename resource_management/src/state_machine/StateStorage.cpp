...
namespace resource_management {
  ...
  StateStorage::StateStorage(uint32_t id,
                             compat::rm_ros::Duration time_out,
                             compat::rm_ros::Time begin_dead_line) : id_(id),
                                                                     time_out_(time_out),
  ...
  {
    if((begin_dead_line_ != compat::rm_ros::Time(0)) && (begin_dead_line_ < compat::rm_ros::Node::get().currentTime()))
      return true;
    else
      return false;
  }
  ...
  void StateStorage::analyse()
  {
    ...
    if(time_out_ == compat::rm_ros::Duration(0))
      std::cout << compat::rm_ros::Node::get().getName() << "[WARNING] state machine " << id_ << ": timeout to 0" << std::endl;

    if(initial_state_ == "")
      std::cout << compat::rm_ros::Node::get().getName() << "[WARNING] state machine " << id_ << ": initial state has no value" << std::endl;

    if(getInitialState() == nullptr)
      std::cout << compat::rm_ros::Node::get().getName() << "[WARNING] state machine " << id_ << ": initial state \"" << initial_state_ << "\" is not defined" << std::endl;

    for(auto state : states_)
    {
      if(state.second->isPartiallyDefined())
        std::cout << compat::rm_ros::Node::get().getName() << "[WARNING] state machine " << id_ << ": state \"" << state.second->getName() << "\" is not defined" << std::endl;

      if(datas_.find(state.second->getName()) == datas_.end())
        std::cout << compat::rm_ros::Node::get().getName() << "[WARNING] state machine " << id_ << ": state \"" << state.second->getName() << "\" do not have data" << std::endl;

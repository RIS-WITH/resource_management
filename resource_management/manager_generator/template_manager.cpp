...
std::map<std::string, std::shared_ptr<resource_management::MessageAbstraction>> $ { class_name }
::stateFromMsg(const ${project_name} _msgs::StateMachineRegister::Request& msg)
{
...
{
  {
    ...
    wrap->setPriority(static_cast<resource_management::ImportancePriority_e>(msg.header.priority.value));
  }
}
!!end
...
std::vector<std::tuple<std::string, std::string, resource_management_msgs::EndCondition>> $ { class_name }
::transitionFromMsg(const ${project_name} _msgs::StateMachine& msg)
{
...
!!for data_type in message_types
    ...
    for(auto x : msg.states_{data_type[0]})
        std::transform(x.header.transitions.begin(), x.header.transitions.end(),
                      ...
                      [x](auto& t){
    {
      return std::make_tuple<std::string, std::string, resource_management_msgs::EndCondition>(
...
${project_name} _msgs::StateMachineRegister::Response $ { class_name }
::generateResponseMsg(uint32_t id)
{
...
::publish{data_type[0]} Msg({data_type[2]} msg, bool is_new)
{
  {
